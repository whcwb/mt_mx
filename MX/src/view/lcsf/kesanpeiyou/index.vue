  ...
  <div class="box_col">
            ...
            <p>
              <Row v-for="(item,index) in AMess" :key="index" style="display: flex;align-items: center;">
                ...
                <Col span="4" :class-name="'colsty'">
                  <Input type="text" size="default" :ref="'input'+(index*3+1)" id="input1"
                         @on-focus="getInputFocus(index*3 +1)" v-model="item.xyXm" placeholder="学员姓名"/>
                </Col>
                <Col span="6" :class-name="'colsty'">
                  <Input type="textarea" :autosize="{minRows: 1,maxRows: 1}" :ref="'input'+(index*3+2)" id="input2"
                         @on-focus="getInputFocus(index*3 + 2)"
                         size="default" v-model="item.bz" placeholder="身份证号码"/>
                </Col>
                <Col span="4" :class-name="'colsty'">
                  <Input type="textarea" :autosize="{minRows: 1,maxRows: 1}" :ref="'input'+(index*3+3)" v-focus="true"
    ...
    <Modal
      ...
      <div slot="header">
        <div class="box_row">
          <div
            style="font-size: 16px;margin-right: 28px;width: 100%;margin-top: 7px;display: flex;justify-content: space-between">
            <h2>培优明细</h2>
            <div>
              <Input placeholder="请输入学员姓名" style="width: 120px;" @on-change="findByXyXm"> </Input>
              <Button style="margin-left: 8px" type="primary" @click="download">下载</Button>
              <Button style="margin-left: 8px" type="primary" @click="downloadNew">下载2</Button>
      ...
      <Row :gutter="32" style="padding-top: 5px">
        <Col span="24">
          <Table size="small" :columns="mxColumns" :data="pymx"></Table>
        </Col>
      </Row>
      <div slot='footer'>
      </div>
...
export default {
  ...
  methods: {
    findByXyXm(e) {
      if (e.data) {
        let jxJx = ''
        this.pymx = this.pymx.filter(item => {
          if (item.jlXm) {
            jxJx = item.jlXm;
          } else {
            item.jlXm = jxJx
          }
          return item.xyXm.indexOf(e.data) != -1
        })
      } else {
        this.pymx = [];
        let jlXm = ''
    ...
    downloadNew() {
      ...
      window.open(this.apis.url + '/api/lcjl/exportMx?token=' + token + "&userid=" + userid + "&" + p);
    },
    save() {//发车
