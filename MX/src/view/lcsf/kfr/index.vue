      ...
      <Form :model="formData" label-position="top">
        ...
        <Row :gutter="32" style="padding-top: 5px" v-if="formData.zddm!=undefined&&formData.zddm.includes('K2PY')">
          <Card>
            <p slot="title">学员信息</p>
            <p>
              <Row v-for="(item,index) in AMess" :key="index" style="margin-top: 10px">
                <Col span="3">
                ...
                <Col span="4" :class-name="'colsty'">
                  <Input type="text" size="default" ref="xyxm" v-model="item.xyXm" placeholder="学员姓名"/>
                </Col>
                <Col span="4" :class-name="'colsty'">
                ...
                <Col span="6" :class-name="'colsty'">
                  <Input type="textarea" :autosize="{minRows: 1,maxRows: 1}"
                         size="default" v-model="item.bz" ref="bz"  placeholder="身份证号码"/>
                </Col>
  ...
  methods: {
    readCardByIndex(item,index) {
      readIdCard.readCardByHand((key, res) => {
        this.$refs.xyxm[index].value = res.Name.trim();
        this.$refs.bz[index].value = res.IDCardNo.trim();
      }, this)
    },
    remove(i) {
